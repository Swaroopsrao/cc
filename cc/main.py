...
def send_image(data):
    try:
        # Assuming 'data' is a FormData object sent by the frontend
        file = data['image']
        
        if file:
            # Upload to Cloudinary (or other services)
            uploaded_image = uploader.upload(file)  # Use appropriate upload method for your service
            image_url = uploaded_image.get('secure_url')

            # Optionally, save image URL to your database or chat record
            chat_record = Chats.query.filter_by(id=session['imageid']).first()
            if chat_record:
                chat_record.message = image_url
                chat_record.image = 1
                db.session.commit()

            # Emit the message to the room or the recipient
            emit('receiveMessage', {
                'message': image_url,  # Send image URL as message
                'username': chat_record.username,
                'image': 1
            }, broadcast=True)

    except Exception as e:
        print(f"Error handling image upload: {str(e)}")
        emit('error', {'message': 'Failed to upload image.'})@socketio.on('message')
